...
def mock_get_render_template():
    with patch("dhcpservd.dhcp_cfggen.DhcpServCfgGenerator._get_render_template", return_value=None) as mock_template:
        yield mock_template


@pytest.fixture
def mock_parse_port_map_alias(scope="function"):
    with patch("dhcpservd.dhcp_cfggen.DhcpServCfgGenerator._parse_port_map_alias", return_value=None) as mock_map, \
